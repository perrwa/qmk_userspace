...
Conventions: layers as named enums with `_` prefix (`_QWERTY`, `_LOWER`). Custom keycodes prefixed `_` (`_LWRSPC`, `_NAVESC`). Dual-function via `LT()`/`LGUI_T()`/`RCTL_T()`. Preserve ASCII-art alignment in LAYOUT macros. RGB settings behind `#ifdef RGB_MATRIX_ENABLE`. C files: 4-space indent, no tabs (`.clang-format`). `.mk`/`Makefile`: tabs (`.editorconfig`).
